...
public class Classroom implements Calculation{
    ...
    @Override
    //Implementasi java lambda expression berupa stream
    public double getMean(){
        return studentsGrade.stream()
   ...
   /*public String getMedian() {
    ...
    //implementasi java lambda dan stream
    public String getMedian() {
        int mid = studentsGrade.size() / 2;
        return studentsGrade.stream()
                .skip(mid)
                .limit(studentsGrade.size() % 2 == 0 ? 2 : 1)
                .reduce((studentsGrade) -> (studentsGrade) / 2)
                .map(Object::toString)
                .orElse("0");
    }
}
